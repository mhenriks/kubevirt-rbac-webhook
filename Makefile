...
IMG ?= ghcr.io/mhenriks/kubevirt-rbac-webhook:latest
...
generate: controller-gen ## Generate code containing DeepCopy, DeepCopyInto, and DeepCopyObject method implementations.
	$(CONTROLLER_GEN) object:headerFile="hack/boilerplate.go.txt" paths="./cmd/...;./internal/..."
...
KIND_CLUSTER ?= kubevirt-rbac-webhook-test-e2e
...
.PHONY: run
run: manifests generate fmt vet ## Run the webhook from your host.
	go run ./cmd/main.go
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name kubevirt-rbac-webhook-builder
	$(CONTAINER_TOOL) buildx use kubevirt-rbac-webhook-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm kubevirt-rbac-webhook-builder
	rm Dockerfile.cross

.PHONY: build-installer
build-installer: manifests generate kustomize ## Generate a consolidated YAML for webhook deployment.
	mkdir -p dist
