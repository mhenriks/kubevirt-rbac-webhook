...
main() {
    ...
    # Check if nested virtualization is available (best effort)
    log_info "Checking virtualization support..."
    if [ -e /dev/kvm ]; then
        log_info "✓ /dev/kvm is available on host"

        # Check and fix /dev/kvm permissions
        if [ ! -r /dev/kvm ] || [ ! -w /dev/kvm ]; then
            log_warn "/dev/kvm exists but may have permission issues"
            log_info "Attempting to fix /dev/kvm permissions..."
            sudo chmod 666 /dev/kvm 2>/dev/null || {
                log_warn "Could not change /dev/kvm permissions (may need sudo)"
                log_warn "If KubeVirt fails, run: sudo chmod 666 /dev/kvm"
            }
        fi

        # Verify permissions
        if [ -r /dev/kvm ] && [ -w /dev/kvm ]; then
            log_info "✓ /dev/kvm is readable and writable"
            log_info "   KubeVirt will use hardware acceleration (fast)"
        else
            log_warn "⚠ /dev/kvm permissions issue - software emulation may be used"
        fi
    else
        log_warn "⚠ /dev/kvm NOT available on host"
        log_warn "   Software emulation will be automatically enabled (VERY SLOW)"
        log_warn "   For CI/GitHub Actions: This is expected and acceptable for basic testing"
        log_warn "   For local dev: Enable virtualization in BIOS and load kernel modules for better performance"
    fi

    # Check if /dev/kvm is accessible in the kind node
    log_info "Checking if /dev/kvm is accessible in cluster nodes..."
    if kubevirtci::kubectl get nodes -o json | grep -q '\"devices.kubevirt.io/kvm\"' 2>/dev/null; then
        log_info "✓ KVM device detected in cluster nodes"
    else
        log_info "ℹ KVM device not detected in cluster nodes (software emulation will be used)"
    fi
